...
- fa83c02: Every compound part is now exported under a flat name as well: `MenuTrigger`, `MenuContent`, `DialogTitle`, `TabsList`, `FieldLabel`, `CardHeader` and so on, from the same entry as its parent (and from the barrel). The dotted form (`Menu.Trigger`) keeps working everywhere it did.
